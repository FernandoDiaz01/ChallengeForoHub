...
public class CursoController {
    ...
    public ResponseEntity<org.springframework.data.domain.Page<DetalleCursoDTO>> listarCursos(@PageableDefault(size = 5, sort = {"id"}) Pageable pageable){
        var pagina = repository.findAll(pageable).map(DetalleCursoDTO::new);
        return ResponseEntity.ok((Page<DetalleCursoDTO>) pagina);
    }
